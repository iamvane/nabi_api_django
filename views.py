...
class LessonRequestView(views.APIView):
    ...
    def post(self, request):
        ...
        if request.user.is_parent():
            ser = sers.LessonRequestSerializer(
                data=data, context={'is_parent': True})
        else:
            ser = sers.LessonRequestSerializer(
                data=data, context={'is_parent': False})
        if ser.is_valid():
            obj = ser.save()
            # to avoid trial_proposed_datetime as string, and get it as datetime
            obj.refresh_from_db()
            lesson = None
            ...
            if lesson:
                ...
                add_to_email_list_v2(request.user, [], ['trial_to_booking'])
            ser = sers.LessonRequestDetailSerializer(
                obj, context={'user': request.user})
...
class LessonRequestItemView(views.APIView):
    ...
    def put(self, request, pk):
        ...
        data['user_id'] = request.user.id
        if request.user.is_parent():
            ser = sers.LessonRequestSerializer(data=data, instance=instance, context={
                                               'is_parent': True}, partial=True)
        else:
            ser = sers.LessonRequestSerializer(data=data, instance=instance, context={
                                               'is_parent': False}, partial=True)
        if ser.is_valid():
            obj = ser.save()
            ser = sers.LessonRequestDetailSerializer(
                obj, context={'user': request.user})
    ...
    def delete(self, request, pk):
        ...
        try:
            ...
            return Response({'message': 'There is not lesson request with provided id'},
                            status=status.HTTP_400_BAD_REQUEST)
        ser = sers.LessonRequestDetailSerializer(
            lesson_request, context={'user': request.user})
        data = ser.data
...
class LessonRequestListView(views.APIView):
    ...
    def get(self, request):
        ...
        qs = LessonRequest.objects.exclude(status=LESSON_REQUEST_CLOSED).annotate(coords=Case(
            When(user__parent__isnull=False, then=F(
                'user__parent__coordinates')),
            When(user__student__isnull=False, then=F(
                'user__student__coordinates')),
            default=None,
            output_field=PointField())
        )
        query_ser = sers.LessonRequestListQueryParamsSerializer(
        ...
        if query_ser.is_valid():
            ...
            if keys.get('location'):
                point = Point(
                    query_ser.validated_data['location'][1], query_ser.validated_data['location'][0], srid=4326)
            ...
            if point and distance is not None:
                ...
                if account and account.coordinates:
                    qs = qs.annotate(distance=Distance(
                        'coords', account.coordinates))
                else:
                    qs = qs.annotate(distance=Distance(
                        'coords', Cast(None, PointField())))
            if keys.get('instrument'):
                qs = qs.filter(
                    instrument__name=query_ser.validated_data.get('instrument'))
            if keys.get('place_for_lessons'):
                bool_filters = reduce(lambda x, y: x | y,
                                      [Q(place_for_lessons=item) for item in query_ser.validated_data.get(
                                          'place_for_lessons')]
                                      )
            ...
            if keys.get('age'):
                ...
                qs = [item for item in qs.all() if item.has_accepted_age(
        ...
        if account:
            ser = sers.LessonRequestItemSerializer(
...
class LessonRequestItemListView(views.APIView):
    ...
    def get(self, request, pk):
        ...
        if not isinstance(request.user, AnonymousUser):
            serializer = sers.LessonRequestListItemSerializer(
                lesson_request, context={'user': request.user})
        else:
...
class ApplicationView(views.APIView):
    ...
    def get(self, request):
        ser = sers.ApplicationListSerializer(Application.objects.filter(
            instructor=request.user.instructor), many=True)
        return Response(ser.data, status=status.HTTP_200_OK)
        ...
        if serializer.is_valid():
            ...
            if user.is_parent():
                if user.parent.tied_students.count() > 1:
                    try:
                        tied_student = TiedStudent.objects.get(
                            id=request.data.pop('studentId'), parent=user.parent)
            ...
            last_lesson = Lesson.get_last_lesson(
                user=user, tied_student=tied_student)
            if not last_lesson:
                return Response({'message': 'Looks like you should create a Trial Lesson first'},
                                status=status.HTTP_400_BAD_REQUEST)
            booking_values_data = get_booking_data_v2(
                user, package_name, last_lesson)
            ...
            with transaction.atomic():
                ...
                booking.description = 'Package {}'.format(
            ...
            task_log = TaskLog.objects.create(task_name='send_booking_invoice', args={
                                              'booking_id': booking.id})
            send_booking_invoice.delay(booking.id, task_log.id)
            if booking.application:
                task_log = TaskLog.objects.create(task_name='send_booking_alert', args={
                                                  'booking_id': booking.id})
...
class AmountsForBookingView(views.APIView):
    ...
    def common(self, request, student_id, package='artist'):
        ...
        if request.user.is_parent():
            ...
            tied_student = None
        last_lesson = Lesson.get_last_lesson(
            user=request.user, tied_student=tied_student)
        ...
        if account.stripe_customer_id:
            try:
                stripe_resp = stripe.PaymentMethod.list(
            ...
            pm_ser = GetPaymentMethodSerializer(
                data=stripe_resp['data'], many=True)
            if pm_ser.is_valid():
            ...
            try:
                stripe_customer = stripe.Customer.create(
                    email=request.user.email, name=account.display_name)
            except Exception as e:
    ...
    def common(self, email, student_id, package):
        ...
        if user.is_parent():
            if user.parent.tied_students.count() > 1:
                try:
                    tied_student = TiedStudent.objects.get(
                        id=student_id, parent=user.parent)
                except TiedStudent.DoesNotExist:
                    ser = MinimalTiedStudentSerializer(
                        user.parent.tied_students.all(), many=True)
                    return Response(ser.data, status=status.HTTP_200_OK)
            else:
                tied_student = user.parent.tied_students.first()
        else:
        ...
        try:
            stripe_customer = stripe.Customer.create(
                email=user.email, name=account.display_name)
        except Exception as e:
            return Response({'message': f'Error creating Customer in Stripe:\n {str(e)}'},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        account.stripe_customer_id = stripe_customer.get('id')
        ...
        if ser.is_valid():
            ...
            if lesson.booking.status == LessonBooking.TRIAL:
                ...
                if lr:
                    ...
                    send_alert_request_compatible_instructors.delay(
                        lr.id, task_log.id)
        ...
        if ser_data.is_valid():
            ...
            if request.data.get('grade'):
                task_log = TaskLog.objects.create(
                    task_name='send_info_grade_lesson', args={'lesson_id': lesson.id})
