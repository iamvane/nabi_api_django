...
class InstructorRegisterReferencesTest(BaseTest):
    ...
    def test_register_references_missing_data(self):
        """Test registration of references"""
        response = self.client.post(self.url, data={'emails': []})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST, msg=response.content.decode())


class ParentRegisterReferencesTest(BaseTest):
    fixtures = ['01_core_users.json', ]
    login_data = {
...
class StudentRegisterReferencesTest(BaseTest):
    fixtures = ['01_core_users.json', ]
    login_data = {
...
class InstructorReferencesListEmptyTest(BaseTest):
    fixtures = ['01_core_users.json', '01_referencerequests.json']
    login_data = {
    ...
    def test_get_list(self):
        """Test getting data list (empty)"""
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        list_response = response.json()['emails']
        list_response.sort()
        self.assertListEqual(list_response, [])
