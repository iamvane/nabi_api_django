...
class InstructorsListTest(BaseTest):
    """Tests for instructors list"""
...
class InstructorsListNoAuthTest(BaseTest):
    """Tests for instructors list, without authenticated user"""
    fixtures = ['01_core_users.json', '02_accounts_instructors.json', '05_lesson_instruments.json',
                '06_accounts_availabilities.json', '07_accounts_educations.json', '08_accounts_employments.json',
                '09_accounts_instructoradditionalqualifications.json', '10_accounts_instructoragegroups.json',
                '11_accounts_instructorinstruments.json', '12_accounts_instructorlessonrates.json',
    ...
    def test_get_data_filters(self):
        """Test getting data using various filters (GET parameters)"""
        response = self.client.get(self.url + '?minRate=11.0&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?maxRate=11.0&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?gender=male&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?availability=monday&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?instruments=flute&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?instruments=flute,piano&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 2)
        self.assertEqual(len(data['results']), 2)
        response = self.client.get(self.url + '?location=29.5845306,-96.1530824&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?studentAges=adults&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?placeForLessons=online&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?qualifications=ear_training&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?gender=male&max_rate=10&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 0)
        self.assertEqual(len(data['results']), 0)
        response = self.client.get(self.url + '?availability=monday,thursday&sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 2)
        self.assertEqual(len(data['results']), 2)
...
class DetailInstructorBasicTest(BaseTest):
    """Test for instructor's detail API endpoint"""
    fixtures = ['01_core_users.json', '02_accounts_instructors.json', '05_lesson_instruments.json', ]
    login_data = {
        'email': 'luisinstruct@yopmail.com',
        'password': 'T3st11ng'
    ...
    def test_get_single_data(self):
        ...
        response = self.client.get(self.url + str(self.current_data['id']) + '/' + '?sort=distance')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
    ...
    def test_get_wrong_id(self):
        ...
        response = self.client.get(self.url + '33/' + '?sort=distance')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND, msg=response.content.decode())


class DetailInstructorTest(BaseTest):
    """Test for getting instructor's details, almost all data"""
    fixtures = ['01_core_users.json', '02_accounts_instructors.json', '06_accounts_availabilities.json',
                '09_accounts_instructoradditionalqualifications.json', '10_accounts_instructoragegroups.json',
                '12_accounts_instructorlessonrates.json', '13_accounts_instructorlessonsizes.json',
                '14_accounts_instructorplaceforlessons.json', '18_phonenumbers.json']
    login_data = {
        'email': 'luisinstruct@yopmail.com',
        'password': 'T3st11ng'
    ...
    current_data = {"id": 1, 'userId': 5, 'displayName': 'Luis I.', 'age': 44, 'interviewed': False,
                    ...
                    'lessonsTaught': 0, 'music': ['piano', 'guitar'],
                    'placeForLessons': {"home": True, "studio": False, "online": True},
                    'qualifications': {"certifiedTeacher": True, "musicTherapy": False, "musicProduction": False,
                                       "earTraining": True, "conducting": False, "virtuosoRecognition": False,
                                       "performance": False, "musicTheory": True, "youngChildrenExperience": False,
    ...
    def test_get_single_data(self):
        """Test getting almost all data of a single entry"""
