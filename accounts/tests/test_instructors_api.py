...
class InstructorsListNoAuthTest(BaseTest):
    ...
    def test_get_data_filters(self):
        ...
        response = self.client.get(self.url + '?minRate=11.0')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?maxRate=11.0')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        ...
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?availability=monday')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?instruments=flute')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?instruments=flute,piano')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 2)
        self.assertEqual(len(data['results']), 2)
        response = self.client.get(self.url + '?location=29.5845306,-96.1530824')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?studentAges=adults')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?placeForLessons=online')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?qualifications=ear_training')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 1)
        self.assertEqual(len(data['results']), 1)
        response = self.client.get(self.url + '?gender=male&max_rate=10')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 0)
        self.assertEqual(len(data['results']), 0)
        response = self.client.get(self.url + '?availability=monday,thursday')
        self.assertEqual(response.status_code, status.HTTP_200_OK, msg=response.content.decode())
        data = response.json()
        self.assertEqual(data['count'], 2)
        self.assertEqual(len(data['results']), 2)
