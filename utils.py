...
def send_alert_request_instructor_ant(instructor, lesson_request, requestor_account):
    ...
    send_email(from_email, [instructor.user.email],
               subject, template, plain_template, params)
...
def send_alert_request_instructor(instructor, lesson_request, requestor_account):
    ...
    if requestor_account.user.is_parent():
        for tied_student in lesson_request.students.all():
            student_details = ', '.join(
                [student_details, f'{tied_student.name}, {tied_student.age} years old ({lesson_request.skill_level})'])
        if student_details:
    ...
    data = {"emailId": settings.HUBSPOT_TEMPLATE_IDS['alert_request'],
            ...
            "customProperties": [
                ...
                {"name": "reference_url",
                    "value": f'{settings.HOSTNAME_PROTOCOL}/request/{lesson_request.id}'},
    ]
    }
    if lesson_request.trial_proposed_datetime:
        ...
        data['customProperties'].append(
            {"name": "lesson_date_subject", "value": f'{date_str} at {time_str}'})
    ...
    data = {"emailId": settings.HUBSPOT_TEMPLATE_IDS['info_lesson_user'],
            ...
            "customProperties": [
                ...
                {"name": "instructor_profile",
                    "value": f'{settings.HOSTNAME_PROTOCOL}/profile/{lesson.instructor.id}'},
    ]
    }
    resp = requests.post(target_url, json=data)
    if resp.status_code != 200:
...
def send_info_lesson_instructor(lesson):
    ...
    data = {"emailId": settings.HUBSPOT_TEMPLATE_IDS['info_lesson_instructor'],
            ...
            "customProperties": [
                {"name": "instructor_name", "value": lesson.instructor.display_name},
                {"name": "lesson_details",
                    "value": f'{student_details.get("name")}, {student_details.get("age")} year old, {instrument_name}'},
                {"name": "schedule_details",
                    "value": f'{date_str} at {time_str} ({time_zone})'},
    ]
    }
    resp = requests.post(target_url, json=data)
    if resp.status_code != 200:
...
def send_invoice_booking(booking, payment):
    ...
    headers = {'Authorization': 'Bearer {}'.format(
        settings.EMAIL_HOST_PASSWORD), 'Content-Type': 'application/json'}
...
def get_booking_data(user, package_name, application):
    ...
    if data.get('freeLesson'):
        data['lessonsPrice'] = application.rate * \
            (PACKAGES[package_name].get('lesson_qty') - 1)
    else:
        data['lessonsPrice'] = application.rate * \
            PACKAGES[package_name].get('lesson_qty')
    # SubTotal is amount to pay if there is not discounts
    # this variable does not include processingFee
    sub_total = data['lessonsPrice'] + data.get('placementFee', 0)
    data['processingFee'] = round(
        Decimal('0.029') * sub_total + Decimal('0.30'), 2)
    # to display, add processing fee
    data['subTotal'] = round(sub_total + data['processingFee'], 2)
    # Now, calculate total
    if data.get('discounts'):
        total = round(data['lessonsPrice'] * (Decimal('100.0000') -
                                              data.get('discounts')) / Decimal('100.0'), 4)
    else:
    ...
    if package_name == 'virtuoso':
        ...
        total = round(total * (Decimal('100.0000') -
                               data['virtuosoDiscount']) / 100, 4)
    total += data.get('placementFee', 0)
    data['processingFee'] = round(
        Decimal('0.029') * total + Decimal('0.30'), 2)
...
def get_booking_data_v2(user, package_name, last_lesson):
    ...
    if data.get('freeLesson'):
        data['lessonsPrice'] = last_lesson.rate * \
            (PACKAGES[package_name].get('lesson_qty') - 1)
    else:
        data['lessonsPrice'] = last_lesson.rate * \
            PACKAGES[package_name].get('lesson_qty')
...
def get_date_time_from_datetime_timezone(datetime_value, time_zone, date_format='%Y-%m-%d', time_format='%H:%M'):
    ...
    localize_datetime = datetime_value.astimezone(
